...
export class ContextMenu {
        ...
        if (this.contextMenuContainerList.length) {
            ...
            const contextMenuDom = findParent(
                ...
                (node: Node & Element) =>
                    !!node &&
                    node.nodeType === 1 &&
                    node.getAttribute(EDITOR_COMPONENT) ===
    ...
    private _render(payload: IRenderPayload): HTMLDivElement {
        ...
        contextMenuContent.classList.add(
            `${EDITOR_PREFIX}-contextmenu-content`,
        );
        // 直接子菜单
            ...
            if (menu.isDivider) {
                ...
                if (c !== 0 && c !== contextMenuList.length - 1) {
                    ...
                    divider.classList.add(
                        `${EDITOR_PREFIX}-contextmenu-divider`,
                    );
                ...
                if (menu.childMenus) {
                    menuItem.classList.add(
                        `${EDITOR_PREFIX}-contextmenu-sub-item`,
                    );
                ...
                if (menu.icon) {
                    icon.classList.add(
                        `${EDITOR_PREFIX}-contextmenu-${menu.icon}`,
                    );
                }
                // 文本
        ...
        const contextMenuWidth =
            contextMenuContainer.getBoundingClientRect().width;
        // 右侧空间不足时，以菜单右上角作为起始点
        const adjustLeft =
            left + contextMenuWidth > innerWidth
