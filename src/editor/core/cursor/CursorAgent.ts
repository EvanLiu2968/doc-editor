            ...
            if (item.kind === 'string') {
                ...
                if (item.type === 'text/html' && isHTML) {
                    item.getAsString((htmlText) => {
                        const pasteElementList = getElementListByHTML(
                            htmlText,
                            {
                        ...
                        if (~startIndex && !rangeManager.getIsSelectAll()) {
                            ...
                            if (
                                anchorElement?.titleId ||
                                anchorElement?.listId
                            ) {
                                let start = 0;
                                while (start < pasteElementList.length) {
                                    const pasteElement =
                                        pasteElementList[start];
                                    if (
                                        VIRTUAL_ELEMENT_TYPE.includes(
                                            pasteElement.type!,
                                        )
                                    ) {
                                        ...
                                        if (pasteElement.valueList) {
                                            for (
                                                let v = 0;
                                                v <
                                                pasteElement.valueList.length;
                                                v++
                                            ) {
                                                const element =
                                                    pasteElement.valueList[v];
                                                if (
                                                    element.value === ZERO ||
                                                    element.value === '\n'
                                                )
                                                    continue;
                                                pasteElementList.splice(
                                                    start,
                                                    0,
                                                    element,
                                                );
