...
function getElementIndexByDragId(dragId: string, elementList: IElement[]) {
    return (<IDragElement[]>elementList).findIndex(
        (el) => el.dragId === dragId,
    );
}
...
export function mouseup(evt: MouseEvent, host: CanvasEvent) {
    ...
    if (host.isAllowDrop) {
        ...
        if (isContainControl) {
            ...
            const cacheStartElement =
                cacheElementList[cacheRange.startIndex + 1];
            ...
            const isAllowDragControl =
                ((cacheStartElement.type !== ElementType.CONTROL ||
                    cacheStartElement.controlComponent ===
                        ControlComponent.PREFIX) &&
                    (cacheEndElement.type !== ElementType.CONTROL ||
                        cacheEndElement.controlComponent ===
        ...
        const cacheRangeEndId = createDragId(
            cacheElementList[cacheRange.endIndex],
        );
        // 设置拖拽值
        ...
        if (
            activeControl &&
            cacheElementList[rangeStart].controlComponent !==
                ControlComponent.POSTFIX
        ) {
        ...
        // 重设选区
        const rangeStartIndex = getElementIndexByDragId(
            rangeStartId,
            elementList,
        );
